...
> **Stale — superseded by TypeScript port.** This implementation plan targets the
> external JavaScript repository `/Users/marcus.kim/repositories/oss/zeroshot-lyo`
> (branch `lyo-integration`, local only). It is a JS-draft plan and is **not**
> the reference implementation for this repository's TypeScript port. The TS port's
> implementation plan is a separate document to be written when that work is scoped.
> See `Specs/00-spec-consolidation.md` §00.5.3 and §00.6 (TypeScript directive).
> Status as of 2026-07-19: **draft code is on disk, unverified.**
Companion spec: `Specs/3-lesson-delta-design.md` (schema, counter rule, status rules).
...
`Specs/3-lesson-delta-design.md`. Checklist:
