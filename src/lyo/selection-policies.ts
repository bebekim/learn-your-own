...
export type SelectionPolicyRef = SelectionPolicy | string | null | undefined;
...
export const THOMPSON_BETA_POLICY: SelectionPolicy = {
  name: 'thompson-beta',
  version: 1,
...
export function policyId(policy: SelectionPolicy): string {
  return `${policy.name}@${policy.version}`;
}

export const DEFAULT_POLICY = THOMPSON_BETA_POLICY;
export const DEFAULT_POLICY_ID = policyId(DEFAULT_POLICY);

// String-addressable policies ('name@version'); object policies can always
// be injected directly without registration.
export const POLICY_REGISTRY = new Map<string, SelectionPolicy>([
  [DEFAULT_POLICY_ID, DEFAULT_POLICY],
]);

// Accepts a policy object, a registry id string, or null (default policy).
export function resolvePolicy(ref: SelectionPolicyRef): SelectionPolicy {
  if (!ref) {
    return DEFAULT_POLICY;
  }
  if (typeof ref !== 'string' && typeof ref.sampleSelection === 'function') {
    return ref;
  }
