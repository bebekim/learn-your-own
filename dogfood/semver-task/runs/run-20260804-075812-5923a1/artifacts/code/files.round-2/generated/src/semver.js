function compareSemver(a, b) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    if (numA && numB) {
      // Both numeric: compare numerically, but if equal numerically, compare as strings
      // to handle cases like "01" vs "1" correctly per spec
      const nA = parseInt(idA, 10);
      const nB = parseInt(idB, 10);
      if (nA !== nB) return nA < nB ? -1 : 1;
      // Numeric values equal - compare as strings for deterministic ordering
